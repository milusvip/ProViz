...
- **右键反推图片**：在任意图片上右键 →「反推此图片的提示词」→ 确认后自动分析
- **截图反推**：右键 →「截图反推提示词」→ 拖选任意区域 → 确认后分析
- **语言选择**：反推前可选中文/英文输出
...
1. 打开扩展设置 → 选择模型并填入 API Key → 保存
2. **反推图片**：在任意网页右键图片 →「反推此图片的提示词」→ 选语言 → 开始反推
3. **截图反推**：右键 →「截图反推提示词」→ 拖选区域 → 确认 → 选语言 → 开始反推
4. 等待分析完成，查看结果
5. 点击扩展图标可查看历史记录
